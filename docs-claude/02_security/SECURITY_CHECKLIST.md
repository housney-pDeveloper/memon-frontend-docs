...
2. 위 체크리스트(섹션 1~7) 항목과 `/security-guidance` 결과를 통합 검토
3. 특히 XSS 방지(섹션 1), 스토리지 보안(섹션 6)은 프론트엔드 특화 항목으로 반드시 병행 점검

### 04_qa-team 수행 시

```
code-reviewer → Critical/Major 0건 확인 (게이트)
    ↓
security-auditor 수행
    ├── /security-guidance 활용 (OWASP 기반 범용 점검)
    ├── 이 체크리스트 기준 프론트엔드 특화 점검
    └── 통합 보안 검수 결과 → result_VERIFICATION.md에 기록
```
